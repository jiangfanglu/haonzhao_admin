class ProfileGroupsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @profile_group.save
        ...
        format.json { render action: 'show', status: :created, location: @profile_group }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to profile_groups_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def profile_group_params
      params.require(:profile_group).permit(:name, :alias)
    end
end
